...
for (pattern in significant_MC_contrib_names) {
	...
	cat(i,"\t",pattern,"\t",full_patt,"\t",significant_contrib[pattern],"\t",get_absolute_count(pattern,names),"\t",get_relative_count(pattern,names),"\t",  file = "analysis_contrib.csv", append=TRUE)
	higher = 100
	nearest_to = ""
